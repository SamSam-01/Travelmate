...
- `front/lib/data/datasources/google_places_remote_data_source.dart` — interroge l'API Google Places REST avec des field masks limités.
...
- `front/test/data/datasources/google_places_remote_data_source_test.dart` — vérifie le field mask économique.
...
- `Google Places API (REST)` — fournit l'autocomplete et les détails de lieu avec un field mask limité aux SKUs Essentials.
...
- L'intégration Google n'utilise pas le SDK natif ni les champs `displayName` / `primaryType` pour éviter d'introduire des coûts Pro. Le nom affiché provient de l'autocomplete, puis les détails sont restreints à `formattedAddress`, `location` et `types`.
